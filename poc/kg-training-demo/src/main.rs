...
fn main() {
    ...
    println!("  Nodes: {}, Edges: {}, Wall time: {}ms",
    ...
    println!("  Weight: mean={:.3}, median={:.3}, max={:.3}, min={:.3}",
        triple_stats.mean_weight, triple_stats.median_weight,
        triple_stats.max_weight, triple_stats.min_weight);
    println!("  Mean co-activations: {:.1}", triple_stats.mean_co_activations);
    println!();
    ...
    for doc in &corpus.documents {
        if let Some(cat) = &doc.category {
            let words: Vec<String> = doc.content.to_lowercase()
    ...
    println!("  Foundation triples: {} (high-weight, same-community)", foundation);
    ...
    let high_weight_same_community = cur.foundation.iter()
        .filter(|t| {
    ...
    println!("  Foundation coherence: {:.1}% same-community", coherence * 100.0);

    // Weight-quality check: mean weight of foundation vs periphery
    let mean_foundation_weight = if cur.foundation.is_empty() { 0.0 } else {
        cur.foundation.iter().map(|t| t.weight).sum::<f64>() / cur.foundation.len() as f64
    };
    let mean_periphery_weight = if cur.periphery.is_empty() { 0.0 } else {
    ...
    println!("  Weight ratio:           {:.1}x",
        if mean_periphery_weight > 0.0 { mean_foundation_weight / mean_periphery_weight } else { 0.0 });
    println!();
    ...
    std::fs::write("poc/kg-training-demo/output/curriculum-ordered.jsonl", &jsonl).ok();

    // Random-ordered JSONL
    let random_jsonl = training_format::to_jsonl_random(&cur, 42);
    std::fs::write("poc/kg-training-demo/output/random-ordered.jsonl", &random_jsonl).ok();
    ...
    csv.push_str(&format!("mean_foundation_weight,{:.4}\n", mean_foundation_weight));
    csv.push_str(&format!("mean_periphery_weight,{:.4}\n", mean_periphery_weight));
    csv.push_str(&format!("triple_coverage,{:.4}\n", triple_stats.total as f64 / stats.graph_nodes.max(1) as f64));
    std::fs::write("poc/kg-training-demo/output/kg-training-benchmark.csv", &csv).ok();
