...
fn main() {
    ...
    for (i, sf) in source_files.iter().enumerate() {
        ...
        println!(
            "    {} → {} elements ({} fns, {} structs, {} traits)",
            sf.relative_path,
            elements.len(),
            elements
                .iter()
                .filter(|e| e.kind == code_digester::CodeElementKind::Function)
                .count(),
            elements
                .iter()
                .filter(|e| e.kind == code_digester::CodeElementKind::Struct)
                .count(),
            elements
                .iter()
                .filter(|e| e.kind == code_digester::CodeElementKind::Trait)
                .count(),
        );
    ...
    println!();
    println!(
        "  Total elements: {} ({} functions, {} structs, {} enums, {} traits)",
        all_elements.len(),
        all_elements
            .iter()
            .filter(|e| e.kind == code_digester::CodeElementKind::Function)
        ...
        all_elements
            .iter()
            .filter(|e| e.kind == code_digester::CodeElementKind::Enum)
            .count(),
        all_elements
            .iter()
            .filter(|e| e.kind == code_digester::CodeElementKind::Trait)
    ...
    let queries = vec![
        ...
        (
            "apoptosis",
            vec!["apoptosis", "death", "signal", "senescence"],
        ),
    ...
    for (query, relevant_terms) in &queries {
        ...
        let graph_retrieved: Vec<String> = graph_results
            .iter()
        ...
        let grep_retrieved: Vec<String> = grep_results.iter().map(|s| s.to_lowercase()).collect();
    ...
    let mean_graph_p5 = if graph_scores.is_empty() {
        0.0
    } else {
        graph_scores.iter().sum::<f64>() / graph_scores.len() as f64
    };
    let mean_grep_p5 = if grep_scores.is_empty() {
        0.0
    } else {
    ...
    println!();
    println!(
        "  Mean P@5: Graph={:.3}, Grep={:.3}",
        mean_graph_p5, mean_grep_p5
    );
    println!(
        "  Session fidelity: nodes={}/{} edges={}/{}",
    ...
    if mean_graph_p5 >= mean_grep_p5 {
        ...
        println!(
            "  Graph retrieval P@5={:.3} vs Grep P@5={:.3}.",
            mean_graph_p5, mean_grep_p5
        );
    ...
    std::fs::write(
        "poc/agentic-memory-demo/output/agentic-memory-benchmark.csv",
        &csv,
    )
    .ok();
